## ============================================================
## Uso: Rscript MC_RDQ2_censored_simulation.R <tau> <censorship>
## Ejemplo: Rscript MC_RDQ2_censored_simulation.R 0.5 0.25
## ============================================================
...
if (length(args) < 2) {
  stop("Debe indicar tau y censorship como argumentos.\n",
       "Uso: Rscript MC_RDQ2_censored_simulation.R <tau> <censorship>\n",
       "Ejemplo: Rscript MC_RDQ2_censored_simulation.R 0.5 0.25", call. = FALSE)
}
...
LogLS <- function(theta, DM, data, tau) {
  ...
  return(sum(dRDQ2(T_F, mu = mu_F, sigma = sigma_F, nu = nu_F, tau = tau, log = TRUE)) +
         sum(pRDQ2(T_C, mu = mu_C, sigma = sigma_C, nu = nu_C, tau = tau, lower.tail = FALSE, log.p = TRUE)))
}
...
system.time(testing.p1 <- mclapply(1:4000, prob,
                                   ...
                                   theta = casos.par[1, ],
                                   tau = tau,
                                   censorship = censorship)) # 8 min approx
system.time(testing.p2 <- mclapply(1:4000, prob,
                                   seeds = seeds,
                                   casos.n = casos.n,
                                   theta = casos.par[2, ],
                                   tau = tau,
                                   censorship = censorship)) # 7 min approx

## --- Carpeta de resultados: RDQ2 / RDQ2_<censura> / tau_<tau> ---
...
results_dir <- file.path(".", "RDQ2", paste0("RDQ2_", cens_str), paste0("tau_", tau_str))
