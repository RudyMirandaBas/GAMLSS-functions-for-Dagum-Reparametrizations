...
dir <- paste("~/Documents/ProyectoTitulo/R/MC/JUST_GAMLSS/results.",
             model[3], sep = "")
setwd(dir)
...
matrix_names <- list(c(paste("beta1", 1:3, sep = ""),
                       paste("beta2", 1:2, sep = ""),
                       paste("beta3", 1:1, sep = "")),
                     c("true", "est_gamlss", "se_gamlss"))

Coefficients <- lapply(lapply(paste("pars_p", rep(1:2, each = 4), "_", rep(casos.n, 2), ".csv", sep = ""), read.csv),
       function(x) matrix(colMeans(x), nr = 6, dimnames = matrix_names))

names(Coefficients) <- paste("B", rep(1:2, each = 4), "_", casos.n, sep = "")

for (i in 1:length(Coefficients)) {
  x <- Coefficients[[i]]
  Coefficients[[i]] <- cbind(x, Bias = x[, 2] - x[, 1], RMSE = MSE[[i]], CP = CP[[i]])
  Coefficients[[i]] <- Coefficients[[i]][, c(1, 2, 4, 3, 5, 6)]
}
